...
const CommonHeader: React.FC<{ selectedKey?: string }> = ({
  ...
  const onRegister = async (values: any) => {
    try {
      ...
      // ✅ Save freelancer info (for FreelancerDashboard header name)
      localStorage.setItem(
        "swachify_registered_user",
        JSON.stringify({
          fullname: `${customerPayload.first_name} ${customerPayload.last_name}`,
          email: customerPayload.email,
          mobile: customerPayload.mobile,
        })
      );

      // ✅ Mark freelancer logged in
      localStorage.setItem("freelancerLoggedIn", "true");
      ...
      // ✅ If user selected "Looking for work" → go to FreelancerDashboard
      if (values.workType === "looking") {
        closeAuthModal();
        navigate("/freelancer-dashboard"); // ✅ your freelancer page route
        return;
      }
